# Contributing to Khoji

Thank you for your interest in contributing to Khoji! We want this tool to be the gold standard for extracting token-efficient website context for LLMs.

## Project Structure

For a deep dive into how Khoji works under the hood, read the [ARCHITECTURE.md](ARCHITECTURE.md) guide.
...
- `src/types/` - The master `KhojiContext` TS interface. **This is the source of truth.**
...
1. **Update Types First:** If your extractor adds new data to the output, update `src/types/KhojiContext.ts` first. This drives the entire project.
...
4. **Wire It Up:** Add your extractor to `Promise.all()` inside `src/pipeline/runner.ts`. Update the final `KhojiContext` object assembly.
